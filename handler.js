...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        // --------- DETECCIÓN DE ROLES SUPER ROBUSTA ---------
        ...
        // Buscar usuario en participantes con múltiples métodos
        let user = {}
        let isRAdmin = false
        let isAdmin = false
        ...
        if (m.isGroup && participants.length > 0) {
            ...
            user = participants.find(u => u.id === sender)
            
            // Método 2: Buscar por número normalizado
            if (!user) {
                user = participants.find(u => normalizeJid(u.id) === senderNum)
            }
            
            // Método 3: Buscar por jid sin dominio
            if (!user) {
                user = participants.find(u => {
                    const uid = u.id.split('@')[0].replace(/[^0-9]/g, '')
                    return uid === senderNum
                })
            }
        ...
        if (m.isGroup && participants.length > 0) {
            ...
            // Método 1: Buscar por JID directo
            bot = participants.find(u => u.id === botJid)
            
            // Método 2: Buscar por número normalizado
            if (!bot) {
                bot = participants.find(u => normalizeJid(u.id) === botNum)
            }
        ...
        // Detección de Owner/Mods/Prems
        const sendNum = sender.replace(/[^0-9]/g, '')
        const isROwner = [conn.decodeJid(global.conn.user.id), ...global.owner.map(([number]) => number)]
            .map(v => v.replace(/[^0-9]/g, ''))
            .includes(sendNum)
        const isOwner = isROwner || m.fromMe
        const isMods = isOwner || global.mods.map(v => v.replace(/[^0-9]/g, '')).includes(sendNum)
        let _user = global.db.data && global.db.data.users && global.db.data.users[m.chat] && global.db.data.users[m.chat][m.sender]
        const isPrems = isROwner || global.prems.map(v => v.replace(/[^0-9]/g, '')).includes(sendNum) || (_user && _user.premium === true)
        // -------------------------------------------------------

        const chat = global.db.data.chats[m.chat] || {}
        const msgHasLink = /(https?:\/\/[^\s]+)/i.test(m.text || '')
        
        if (m.isGroup && chat.onlyAdmin && !isAdmin && !(chat.antiLink || chat.antiLinkAll)) {
            return
        }
        
        if (m.isGroup && chat.onlyAdmin && !isAdmin && (chat.antiLink || chat.antiLinkAll) && !msgHasLink) {
            return
        }
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    let ppwel = "https://files.catbox.moe/t8m8fc.jpg"
        ...
        case 'demote':
            if (chat.detect) {
                text = (action === 'promote' ? 
                    (chat.sPromote || this.sPromote || conn.sPromote || '@user ahora es admin') : 
                    (chat.sDemote || this.sDemote || conn.sDemote || '@user ya no es admin'))
                        .replace('@user', '@' + participants[0].split('@')[0])
                
                let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://i.ibb.co/2WzLyGk/profile.jpg')
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.icon) {
            text = (chats.sIcon || this.sIcon || conn.sIcon || 'El icono del grupo cambió')
        }
        if (groupUpdate.revoke) {
            text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'El enlace del grupo cambió a\n@revoke')
